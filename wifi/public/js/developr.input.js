...
;(function($, window, document, undefined)
{
	...
	$.fn.styleCheckable = function(options)
	{
		...
		return this.each(function(i)
		{
			...
			{
				replacement = $('<span class="'+this.className+checked+disabled+' replacement"'+title+' tabindex="0">'+
	...
	$.fn.styleSelect = function(options)
	{
		...
		return this.each(function(i)
		{
			var element = $(this),
				...
				title = (this.title && this.title.length > 0) ? ' title="'+this.title+'"' : '',
				width, widthString, select, dropDown, text;
			...
			{
				// Create
				select = $('<span class="'+this.className.replace(/validate\[.*\]/, '').replace(/(\s*)select(\s*)/, '$1selectMultiple$2')+disabled+' replacement"'+title+widthString+' tabindex="0">'+
			...
			{
				// Selected text, must not be empty to preserve vertical-align
				...
				// Create
				select = $('<span class="'+this.className.replace(/validate\[.*\]/, '')+disabled+' replacement"'+title+widthString+' tabindex="0">'+
	...
	function _refreshSelectValues()
	{
		...
		if (list.length > 0 && replaced)
		{
			...
			list.empty();
			replaced.children().each(function(i)
			{
				var classes = [],
					text = $(this).text();
				...
				$('<span'+((classes.length > 0) ? ' class="'+classes.join(' ')+'"' : '')+'>'+checkList+text+'</span>')
					.appendTo(list)
					.data('select-value', this);
			});
			list.children().on('touchend click', _clickSelectValue);
	...
	function _updateSelectValueText(select, values, dataText, defaultText)
	{
		...
		{
			select.children('.select-value').removeClass('alt').text((values.length > 0) ? values.join(', ') : '&nbsp;');
		}
		else
		{
	...
	{
		var replaced = $(this),
			selected = replaced.children(':selected'),
	...
	doc.on('focus', 'span.select, span.selectMultiple', function(event)
	{
		...
		if (blurTimeout)
		{
			// The select is about still focused but about to blur, prevent and remain focused
		...
		function onBlur()
		{
			...
			if (!blurTimeout)
			{
				...
				select.data('selectBlurTimeout', setTimeout(onBlur, 40));
				return;
			}
			else
			{
